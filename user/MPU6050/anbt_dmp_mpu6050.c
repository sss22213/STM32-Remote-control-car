...
u8 AnBT_DMP_MPU6050_Init(void)	
{
	...
	USART_Sendstring(USART1,"P-1,Finish DMP device init."); 		//完成初始化
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL))USART_Sendstring(USART1,"P-2,Finish DMP set sensor.");		
	else USART_Sendstring(USART1,"P-2,DMP set sensor error.");
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) USART_Sendstring(USART1,"P-3,Finish DMP configure fifo.");		
	else USART_Sendstring(USART1,"P-3,DMP configure fifo error.");
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ))USART_Sendstring(USART1,"P-4,Finish DMP set sample rate.");			
	else USART_Sendstring(USART1,"P-4,DMP set sample_rate error.");
	//
	if(!dmp_load_motion_driver_firmware())USART_Sendstring(USART1,"P-5,Finish DMP load motion driver firmware.");				
	else USART_Sendstring(USART1,"P-5,DMP load motion driver firmware error.");
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation))) USART_Sendstring(USART1,"P-6,Finish DMP set orientation.");		
	else USART_Sendstring(USART1,"P-6,DMP set orientation error.");
	//
	if(!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
	DMP_FEATURE_ANDROID_ORIENT | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
	DMP_FEATURE_GYRO_CAL))	USART_Sendstring(USART1,"P-7,Finish DMP enable feature.");		
	else USART_Sendstring(USART1,"P-7,DMP enable feature error.");
	//
	if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ))USART_Sendstring(USART1,"P-8,Finish DMP set fifo rate.");		
	else USART_Sendstring(USART1,"P-8,DMP set fifo rate error.");
	//
	run_self_test();
	if(!mpu_set_dmp_state(1)) USART_Sendstring(USART1,"P-9,Finish DMP set dmp state.");				
	else USART_Sendstring(USART1,"P-9,DMP set dmp state error.");					
	//
	return 0;
}
